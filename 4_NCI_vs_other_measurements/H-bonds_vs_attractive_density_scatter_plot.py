...
ensemble_weights = {'Folded': 820, 'Transition': 7, 'Unfolded': 161}
...
# Now we extract the same data from folding and unfolding events
replica_directories = sorted(
    [os.path.join('.', d) for d in os.listdir('.') if os.path.isdir(os.path.join('.', d)) and ('_unfolding_' in d or '_folding_' in d)],
    key=lambda x: (int(re.search(r'event_(\d+)', x).group(1)), '_folding_' in x))

for replica_dir in replica_directories:
    config.read(os.path.join(replica_dir, 'relevant_data.txt'))
    topology_file = config.get('Settings', 'topology_file')
    trajectory_file = config.get('Settings', 'trajectory_file')
    sampling_step = config.getint('Settings','Sampling step')
    ...
    hbonds = np.array([len(md.baker_hubbard(frame, exclude_water=True, periodic=True, sidechain_only=False)) for frame in event_traj])
    NCI_att_data = np.zeros(len(event_traj))

    replica_csvs_path = os.path.join(replica_dir, 'NCI_csvs')
    for filename in os.listdir(replica_csvs_path):
        if '_densities' in filename and filename.endswith('.csv'):
            NCI_df = pd.read_csv(os.path.join(replica_csvs_path, filename))
            NCI_att_data += NCI_df['Attractive n^1'].to_numpy()

    # Each transition has the first and last 10 frames correspond to Folded or Unfolded depending on the transition type, 
    unfolded_frames = range(10) if '_folding_' in replica_dir else range(len(event_traj) - 10, len(event_traj))
    folded_frames = range(len(event_traj) - 10, len(event_traj)) if '_folding_' in replica_dir else range(10)
    #transition_frames = range(10, len(event_traj) - 10) #the rest of the points are transition but they are unnecessary for a varied sampling 
                                                        # To reduce the computational load we ignore this part 
                                                        
    for idx in folded_frames:
        ensemble_data['Folded'].append((hbonds[idx], NCI_att_data[idx]))
    for idx in unfolded_frames:
        ensemble_data['Unfolded'].append((hbonds[idx], NCI_att_data[idx]))
    #for idx in transition_frames:
    #    ensemble_data['Transition'].append((hbonds[idx], NCI_att_data[idx]))
