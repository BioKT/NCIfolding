...
traj_top = md.load_pdb(topology_file).topology

# Additionally, we load the Alphafold pdb that we will use to calculate native contacts
native_state = md.load_pdb(native_state_file)
native_state_top = md.load_pdb(native_state_file).topology 
...
for root, dirs, files in os.walk(trajectory_directory):
    for file in files:
        if file.endswith('.dcd'):
            dcd_file_list.append(os.path.join(root, file))
...
def DESHAW_q(traj, native):
    ...
    #r = np.zeros((n_frames, len(native_contacts)))
    #for frame in range(n_frames):
    #    for i, pair in enumerate(native_contacts):
    #        r[frame, i] = np.linalg.norm(traj.xyz[frame, pair[0], :] - traj.xyz[frame, pair[1], :])

    # Compute native distances (r0)
    r0 = md.compute_distances(native[0], native_contacts)
    #r0 = np.array([np.linalg.norm(native.xyz[0, pair[0], :] - native.xyz[0, pair[1], :])for pair in native_contacts])
...
for dcd_file in dcd_file_list: 
    traj = md.load_dcd(dcd_file, top=traj_top)
